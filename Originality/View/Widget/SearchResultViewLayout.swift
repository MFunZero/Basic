...
class SearchResultViewLayout: UICollectionViewLayout {
    ...
    */
    override func collectionViewContentSize() -> CGSize {
         var height = ((self.collectionView!.bounds.width) / CGFloat(countInLine) ) * 1.8 * CGFloat(self.collectionView!.numberOfItemsInSection(0) / countInLine) + 60
        ...
        if result == 1{
            height = height+(self.collectionView!.bounds.width) / CGFloat(countInLine) * 1.8
        }
    ...
    override func layoutAttributesForSupplementaryViewOfKind(elementKind: String, atIndexPath indexPath: NSIndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if elementKind == UICollectionElementKindSectionHeader {
            ...
            var height = ((self.collectionView!.bounds.width) / CGFloat(countInLine) ) * 1.8 * CGFloat(self.collectionView!.numberOfItemsInSection(0) / countInLine)
            let result = self.collectionView!.numberOfItemsInSection(0) % (countInLine)
//            let resultA = self.collectionView!.numberOfItemsInSection(0) / (countInLine)
