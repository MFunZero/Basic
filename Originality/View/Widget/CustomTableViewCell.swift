...
class CustomTableViewCell: UITableViewCell {
    ...
    var fileName:String?{
        willSet{
            self.picture.image = UIImage(named: defaultHeaderImage)
        }
        didSet {
    ...
    override func drawRect(rect: CGRect)
    {
        ...
        detailLabel.frame = CGRect(x: titleLabel.frame.origin.x, y: self.frame.height / 2 - 2, width: self.frame.width - picture.frame.width - 30, height: self.frame.height / 2 - 2)
