...
class UserCollectionLayout: UICollectionViewLayout {
    ...
    */
    override func collectionViewContentSize() -> CGSize {
        var height = ((self.collectionView!.bounds.width) / CGFloat(countInLine) ) * 1.5 * CGFloat(self.collectionView!.numberOfItemsInSection(0) / countInLine) + 220
        let result = self.collectionView!.numberOfItemsInSection(0) % (countInLine)
        
        
        if result == 1{
            height = height+(self.collectionView!.bounds.width) / CGFloat(countInLine) * 1.5
        }
    ...
    override func layoutAttributesForSupplementaryViewOfKind(elementKind: String, atIndexPath indexPath: NSIndexPath) -> UICollectionViewLayoutAttributes? {
        ...
        if elementKind == UICollectionElementKindSectionHeader {
            ...
            var height = ((self.collectionView!.bounds.width) / CGFloat(countInLine) ) * 1.5 * CGFloat(self.collectionView!.numberOfItemsInSection(0) / countInLine) + 160
            let result = self.collectionView!.numberOfItemsInSection(0) % countInLine
