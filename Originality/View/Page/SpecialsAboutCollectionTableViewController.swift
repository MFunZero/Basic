//
//  SpecialsAboutCollectionTableViewController.swift
//  Originality
//
...
class SpecialsAboutCollectionTableViewController: UITableViewController {
...
getSpecialMessage(self.specials[indexPath.row]) { (sp) in
    if sp.pictureName != nil {
    cell.fileName = sp.pictureName
    }
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        getSpecialMessage(self.specials[tag]) { (spInfo) in
            vc.userId = spInfo.userId
            self.getUserMessage(spInfo.userId, callback: { (user) in
                vc.user = user
                if user.avatar != nil {
                ...
                self.getData(user.objectId) { (singleData) -> Void in
                    ...
                    if (self.navigationController!.topViewController!.isKindOfClass(SpecialsAboutCollectionTableViewController)) {
...
extension SpecialsAboutCollectionTableViewController {
    func getUserMessage(id:String,callback:(UserInfo -> Void)){
    ...
    func getSpecialMessage(id:String,callback:(SpecialInfo -> Void)){
        let bquery = BmobQuery(className: "special")
        bquery.includeKey("userId")
        bquery.getObjectInBackgroundWithId(id) { (obj, error) in
            let sp = SpecialInfo()
            if error != nil{
                print("error")
            }
                ...
                sp.userId = user.objectId
                sp.username = user.username
                if let avatar = user.objectForKey("avatar") {
                sp.userAvator = avatar as! String
                
                }
                
                if let pictureName = obj.objectForKey("mainPicture") {
                    sp.pictureName = pictureName as! String
                }
                
                sp.objectId = obj.objectId
                
                if let title = obj.objectForKey("title") {
                    sp.title = title as! String
                }
                else {
                    sp.title = ""
                }
