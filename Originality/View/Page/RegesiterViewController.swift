...
class RegesiterViewController: UIViewController ,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.title = "注册"
        let leftButton = UIBarButtonItem(image: UIImage(named: "grey_left"), style: UIBarButtonItemStyle.Done, target: self, action: #selector(RegesiterViewController.back(_:)))
...
//        self.presentViewController(nav, animated: true, completion: nil)
    ...
    func configView()
    {
        ...
        self.loginButton.addTarget(self, action: #selector(RegesiterViewController.regesiter), forControlEvents: .TouchUpInside)
        self.loginButton.enabled = false
...
//                if isSuccessful {
                    BmobUser.signOrLoginInbackgroundWithMobilePhoneNumber(phoneNumber, andSMSCode: smsCode, block: { (user, error) in
                        if error != nil {
                            NSLog("%@", error)
                            ...
                            let vc = SettingPasswordAndUsernameController()
                            vc.objId = objId
                            self.navigationController?.pushViewController(vc, animated: true)
                        }
                    })
